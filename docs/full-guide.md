...
A 股个股分析的 LLM 输入在既有技术面、财报、资金流之外，还包含两类基本面增强数据（fail-open，取数失败不阻断分析）：估值参照（个股 PE/PB 的 3/5 年历史分位 + 东财行业 PE/PB 中位数，行业中位数为动态市盈率口径、剔除亏损股）与结构性风险（未来 90 天限售解禁、股权质押比例、融资融券余额及近似 5 个交易日变化）。财报区块同时提供营收/归母净利同比与净现比（经营现金流/归母净利润，单期口径）。Prompt 硬约束要求：估值高低结论必须引用估值参照数据，估值参照缺失时禁止输出“估值合理/偏低/偏高”类判断；解禁/质押/两融数据缺失时不得声称“无解禁压力/无质押风险”。
