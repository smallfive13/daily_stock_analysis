...
## [Unreleased]
- [chore] 同步上游截至 2026-09-19 的 main（f9e7b77），保留本 fork 的 A 股复盘风险门槛、新闻时效过滤和结构风险字段，并补充富途港股上下文与无代码指数新闻的合并回归测试。
...
- [新功能] A 股个股分析新增“估值参照”数据块：PE/PB 的 3/5 年历史分位与东财行业 PE/PB 中位数注入分析 prompt，并硬约束估值结论必须引用该数据，缺失时禁止输出“估值合理/偏低/偏高”类判断。
- [新功能] A 股个股分析新增“结构性风险”数据块：未来 90 天限售解禁、股权质押比例、融资融券余额及变化注入分析 prompt（fail-open，缺失时要求按“数据缺失”处理并禁止编造）。
- [改进] 财报与分红区块补充营收同比、归母净利同比与净现比（经营现金流/归母净利润），辅助识别利润质量。
- [新功能] A 股大盘复盘新增"外围市场联动"数据块：标普500、纳斯达克、费城半导体、日经225、KOSPI、恒生、美元指数注入复盘 prompt（yfinance 主源 + 东财全球指数兜底，fail-open），并要求复盘先判断当日走势是否由外围事件驱动、解释板块联动传导链。
- [改进] A 股大盘复盘新闻检索升级：固定检索词加入"隔夜美股全球市场大事件"，并按当日领涨领跌板块动态生成事件导向检索词（如"半导体 板块 大跌 原因"），检索结果按 URL 去重。
- [改进] 大盘复盘消息催化区分"新闻检索服务未配置"与"已检索无结果"两种状态并给出显式提示，替代笼统的"暂无相关新闻"，避免把配置缺失误读为市场无消息。
- [新功能] A 股大盘复盘 prompt 新增板块资金流、涨停结构与指数关键位输入，并要求明日计划触发条件引用可观察锚点。
- [新功能] A 股大盘复盘新增版本化 `a_share_evidence`：日期化采集涨停、炸板、昨日涨停与跌停池，计算指数 MA5/MA10/MA20、炸板率、昨日涨停溢价、连板梯队和疑似一字板比例，生成题材候选与核心观察票，并将结构化风险门槛贯通历史 payload、模板降级报告和个股 Agent 大盘护栏。
- [测试] 增加 A 股复盘事件池 fallback、字段归一化、情绪/题材评分、风险门槛、结构化 payload 与 DailyMarketContext 白名单裁剪回归测试。
- [修复] A 股 Market Light 将市场热度与确定性风险状态拆分，五指数均线、情绪质量、容量核心及外围科技门槛可否决高热度下的进攻结论，并把数字仓位上限统一传递到 Prompt、Markdown、通知和个股大盘护栏。
- [改进] A 股大盘复盘新增单一规范化快照与持久化前一致性校验，统一日期化情绪口径、限制最多三个可操作方向，并把一字或严重缩量观察票固定为零仓位的情绪温度计。
- [测试] 新增 2026-07-29 固定场景，覆盖 81 涨停/14 炸板/9 跌停、五指数 MA、外围科技红灯否决、主题降级、观察票交易资格和通知/下游一致性。
- [文档] 新增 A 股大盘复盘数据、风险、兼容和回滚契约，并同步修正中英文完整指南与 Market Light 告警字段说明。
