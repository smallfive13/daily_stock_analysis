...
> - CN market reviews use a nine-section post-market workstation layout. Market heat is reported separately from the deterministic risk state, followed by index/legacy-theme/external risk, prohibited setups, at most three actionable directions, upgrade conditions, watchlist eligibility, numeric position caps, and unverified items. Heat describes participation and activity only; it no longer implies risk-on positioning. The risk state is constrained by MA5/MA10/MA20 coverage for five major indices, sentiment quality, capacity-core/tradeable-front confirmation, and timestamped external technology data.
> - CN reviews emit versioned `a_share_evidence` and `normalized_review_snapshot` contracts. Date-scoped event pools override approximate overview limit counts; all five major indices are actively requested; Nasdaq and a semiconductor proxy form the external-technology gate; one-price or severely illiquid names are observation-only with a zero cap. Prompt, Markdown, persisted payload, notifications, and `daily_market_context` consume the same snapshot. A pre-persistence consistency guard replaces contradictory model Markdown with a conservative deterministic template. See [A-share market-review data and risk contract](a-share-market-review-contract.md) for fields, thresholds, compatibility, and rollback details.
...
For China A-share stocks, the LLM analysis input also carries two fundamental enrichment blocks (fail-open; fetch failures never block the analysis): a valuation reference block (the stock's own 3y/5y PE/PB historical percentiles plus East Money industry PE/PB medians; industry medians use the dynamic-PE metric with loss-making constituents excluded) and a structural risk block (restricted-share releases within 90 days, share-pledge ratio, and margin financing balance with an approximate 5-trading-day change). The earnings table additionally provides revenue/net-profit YoY and the operating-cash-flow to net-profit ratio (single reporting period). Prompt hard constraints require valuation conclusions to cite the valuation reference data — when it is missing, the model must not output "valuation is reasonable/low/high" style judgements — and forbid claiming "no unlock pressure / no pledge risk" when structural risk data was not fetched.
