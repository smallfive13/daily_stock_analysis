...
> - CN market reviews also emit versioned `a_share_evidence`. Limit-up, failed-board, previous-limit and limit-down pools are requested for the effective trade date; local index history provides MA5/MA10/MA20 and the 20-day range. The evidence contract derives failed-board ratio, previous-limit premium, continuation ladder, a one-price-board heuristic, ranked themes and bounded watchlist candidates, and persists them with market-review history. With `DAILY_MARKET_CONTEXT_ENABLED=true`, only allowlisted sentiment fields, the top three themes and top five candidates reach stock-analysis prompts; triggered risk gates reuse the existing conservative-position guardrail. Any date-scoped provider failure is reported through `data_quality` and remains fail-open for the rest of the review and other markets.
